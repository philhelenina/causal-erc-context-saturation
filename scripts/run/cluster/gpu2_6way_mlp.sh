#!/bin/bash
###############################################################################
# GPU 2: 6way MLP (BERT + RoBERTa, n=10 seeds)
# 2 encoders × 2 layers × 3 pools × 10 seeds = 120 runs
###############################################################################

export CUDA_VISIBLE_DEVICES=2

SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
TRAIN_SCRIPT="${BASE_DIR}/scripts/train/train_npz_classifier_6way.py"
...
TASK="6way"
MODEL="mlp"
ENCODERS=("bert-base" "roberta-base")
LAYERS=("avg_last4" "last")
POOLS=("wmean_exp_fast" "wmean_exp_med" "wmean_exp_slow")
...
echo "==========================================="
echo "GPU 2: 6way MLP (BERT + RoBERTa, n=10)"
echo "==========================================="
...
mkdir -p "${SUMMARY_DIR}"
SUMMARY_FILE="${SUMMARY_DIR}/gpu2_6way_mlp.txt"
echo "GPU 2: 6way MLP (n=10)" > "${SUMMARY_FILE}"
