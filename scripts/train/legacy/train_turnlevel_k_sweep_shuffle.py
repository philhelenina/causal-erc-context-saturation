...
def build_windows_by_dialogue(X, y, file_ids, k, shuffle_past=False):
    """
    Build K-window inputs preserving dialogue structure.
    shuffle_past: if True, shuffle past K utterances (keep current at end)
    """
    Xw, yw = [], []
    ...
    for fid in np.unique(fids):
        ...
        for i, idx in enumerate(idxs):
            ...
            # SHUFFLE LOGIC HERE
            if shuffle_past and k > 0 and seq.shape[0] > 1:
                current = seq[-1:]  # Keep current utterance (last one)
                past = seq[:-1]     # Past K utterances
                np.random.shuffle(past)  # Shuffle past
                seq = np.vstack([past, current])  # Reconstruct with current at end
...
def main():
    ...
    parser.add_argument("--shuffle", action="store_true",
                       help="Shuffle past K utterances (keep current at end)")
    ...
    for k_idx, k in enumerate(k_values):
        ...
        if args.shuffle:
            print(f"  🔀 SHUFFLE MODE ENABLED")
        print(f"{'='*80}")
        
        # Build windows - shuffle 옵션 전달
        Xtr_k, ytr_k = build_windows_by_dialogue(X_tr, y_tr, fid_tr, k, shuffle_past=args.shuffle)
        Xva_k, yva_k = build_windows_by_dialogue(X_va, y_va, fid_va, k, shuffle_past=args.shuffle)
        Xte_k, yte_k = build_windows_by_dialogue(X_te, y_te, fid_te, k, shuffle_past=args.shuffle)
