...
# -*- coding: utf-8 -*-
"""
train_npz_classifier_4way.py (VERBOSE VERSION)
- IEMOCAP 4-way 전용
- SRoBERTa (또는 SenticNet-fused) 임베딩을 불러와 학습/평가
- 실시간 성능 출력 추가
"""
import os
...
DEFAULT_EMB_ROOT = HOME / "data" / "embeddings" / "4way" / "sentence-roberta"
CSV_BASE = HOME / "data" / "iemocap_4way_data"
CSV_NAME = "{split}_4way_unified.csv"

LABEL_TO_ID = {"ang":0,"hap":1,"sad":2,"neu":3}
...
def main():
    ...
    print(f"\n{'#'*60}")
    print(f"🚀 Starting 4-way Classification")
    print(f"{'#'*60}")
    ...
    print(f"{'='*60}")
    eval_and_save(args,model,Xte,yte,Path(args.out_dir),f"sr4_{args.layer}_{args.pool}")
