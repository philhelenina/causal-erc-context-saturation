#!/bin/bash
###############################################################################
# Hierarchical GPU 2: 6way (mean, lstm, attn-mlp aggregators)
...
echo "==========================================="
echo "Hierarchical GPU 2: 6way (n=10)"
echo "==========================================="
...
mkdir -p "${SUMMARY_DIR}"
SUMMARY_FILE="${SUMMARY_DIR}/hier_gpu2_6way_summary.txt"
echo "Hierarchical GPU 2: 6way (n=10)" > "${SUMMARY_FILE}"
...
for layer in "${LAYERS[@]}"; do
    for pool in "${POOLS[@]}"; do
        ...
        # attn aggregator: mlp only (lstm will run on GPU3)
        agg="attn"
        clf="mlp"
        for seed in "${SEEDS[@]}"; do
            ((count++))
...
echo ""
echo "==========================================="
echo "✅ Hierarchical GPU 2 Complete (6way)"
echo "==========================================="
