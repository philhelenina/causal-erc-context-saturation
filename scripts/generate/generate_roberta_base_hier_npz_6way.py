...
# -*- coding: utf-8 -*-
"""
generate_roberta_base_hier_npz_6way.py
...
- 저장: {OUT_ROOT}/roberta-base-hier/{layer}/{pool}/{split}.npz
"""
import os
...
OUT_ROOT_DEFAULT = HOME / "data" / "embeddings" / "6way" / "roberta-base-hier"
...
def idf_weights(token_ids: torch.Tensor, attn_mask: torch.Tensor, tokenizer, idf_map: Dict[str, float]) -> torch.Tensor:
    ...
    for b in range(B):
        ...
        for tid, m in zip(ids, msk):
            ...
            t = t.replace("Ġ","").lower()  # RoBERTa uses Ġ instead of ##
...
def main():
    ...
    for split in args.splits:
        df = pd.read_csv(Path(args.data_dir) / f"{split}_6way_unified.csv")
